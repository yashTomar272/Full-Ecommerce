...
export default function ProductInfo() {
  ...
  const { addToCart, removeFromCart, getProductById, user, database } =
    firebase;
    ...
    if (user) {
      const fetchCartItems = async () => {
        const cartSnapshot = await getDocs(
          collection(database, `users/${user.uid}/cartItems`),
        );
  ...
  return (
    <>
      <Layout>
        <div
          className="Info_main bg-light dalju m "
          style={{ heihgt: "900px" }}
        >
          <div className=" gap-5 flex-wrap d-flex judtify-content-center">
            <div
              className="Info_img bg-dalju p-5 "
              ...
              <button
                ...
                onClick={() => {
                  if (product && product.id) {
                    ...
                    console.error(
                      "Product data or ID is not available",
                      product,
                    );
                  }
                }}
